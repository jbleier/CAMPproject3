...
pi = np.pi
from brian2 import *
from sklearn.linear_model import LogisticRegression as LReg # "Support vector classifier"
model = LReg()
...
def plot_decision_function(model, ax=None, color='black'):
    """Plot the decision function for a 2D LReg"""
    if ax is None:
        ax = plt.gca()
    xlim = ax.get_xlim()
    ylim = ax.get_ylim()
